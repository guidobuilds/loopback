...
`@guidobuilds/loopback-setup` is the **ephemeral, one-shot installer** for loopback. It
registers the **remote** loopback MCP endpoint, copies the `feedback-detector`
skill and the `/harness-feedback` command into your harness, saves your service
URL + token — then exits. There is no MCP server bundle to download; the MCP is
hosted by the loopback service.
...
2. Registers the **remote** loopback MCP endpoint (`<service-url>/mcp`) + the
   `Authorization: Bearer <token>` header with the chosen agent
   (`claude mcp add-json` / `opencode.json(c)` / `~/.codex/config.toml`).
3. Copies the detector skill + slash-command into the agent's user directories.

Then restart your agent. **No hooks are injected** into your agent's settings, and
**no MCP server bundle is downloaded** — the MCP is hosted by the service.
...
- The remote MCP server is registered.
...
# Also delete ~/.loopback/ (the saved credentials).
...
state. `~/.loopback/config.json` is the installer's single source of truth for the
service URL + token; since the MCP is now **remote**, the installer also writes the
token into each agent's MCP registration as an `Authorization: Bearer` header
(that is how the remote server authenticates the request). It does not write
`LOOPBACK_*` `env` blocks.

### Claude Code

- Registers the **remote** MCP at **user scope** via `claude mcp add-json loopback
  '{"type":"http","url":"<service>/mcp","headers":{"Authorization":"Bearer …"}}'
...
- Adds a remote `mcp.loopback` entry (`{ "type": "remote", "url": "<service>/mcp",
  "enabled": true, "headers": { "Authorization": "Bearer …" } }`) to
  `opencode.jsonc` (preferred if present) else `opencode.json` under
  `$XDG_CONFIG_HOME/opencode` (or `~/.config/opencode`). Custom headers you add by
  hand are preserved.
...
- Adds an `[mcp_servers.loopback]` block to `~/.codex/config.toml` with the remote
  `url` and an `http_headers = { "Authorization" = "Bearer …" }` table. If you have
  manually added env entries under `[mcp_servers.loopback.env]`, they are preserved
  across re-runs.
...
- **MCP tools not appearing in Claude Code** — confirm registration with
  `claude mcp list` / `claude mcp get loopback` (check the remote URL), then
